...
> Before planning any work, read the [Notion Дорожная карта]([redacted])
> (parent page: [🧮 Morphogenesis-GA]([redacted])).
> An older 5-phase "technical subcard" used to live in this file — it was a
> narrow refactor-only view and **does not match the real product roadmap**.
> See the [Roadmap](#roadmap-source-of-truth-notion) section below for the
> authoritative 7-phase map and current status.
...
An older version of this file contained a 5-phase "Roadmap Context" that
read:
> _Phase 0: Code audit · Phase 1: Refactor run_train.py · Phase 2: NSGA-II +
> adaptive mutation + HPO · Phase 3: Test hardening · Phase 4: v1.0 release_

That was a **narrow technical subcard** used during the initial refactor
work and was incorrectly presented as the canonical roadmap. It is now
superseded by the Notion roadmap referenced at the top of this file. If a
session starts quoting that old numbering (especially "Phase 2 = NSGA-II"),
that is a signal the session is working from stale context and should
re-read the Notion page before planning.
